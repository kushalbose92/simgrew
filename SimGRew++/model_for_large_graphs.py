...
class SimGRewGCN(nn.Module):
    ...
    def forward(self, x, adj_matrix):
        ...
        A_hat = self.estimate_adjacency_mask(x, adj_matrix, self.prob)
        updated_adj_matrix = A_hat
        updated_edge_index = torch.where(updated_adj_matrix != 0)
        new_adj = updated_adj_matrix.reshape(m*m)
        edge_weights = new_adj[new_adj != 0]
        updated_edge_index = torch.stack([updated_edge_index[0], updated_edge_index[1]])
        # # print(updated_edge_index[0][:10])
        total_edges = adj_matrix.sum()
        # total_edges = adj_matrix.coalesce().values().shape[0]
        new_edges = len(edge_weights)
        
        # total_edges = torch.nonzero(adj_matrix).sum()
        # new_edges = torch.nonzero(A_hat).sum()
        # updated_adj_matrix = self.normalize_adj(A_hat)
        ...
        # scale_factor = (norm_weights * new_edges)

        
        # updated_adj_matrix = self.normalize_adj(adj_matrix)
        
        # message propagation through hidden layers
        for i in range(self.num_layers):
         
            x = self.gcn_convs[i](x, updated_adj_matrix)
            ...
            # dir_energy = self.dirichlet_energy_with_adjacency(updated_adj_matrix, x)
            # print(de)
